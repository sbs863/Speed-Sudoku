...
Lobby.prototype.createGame = function(){
   ...
   var sudokuGame = new SudokuGame(gameId); 
    console.log("game object in lobby" + JSON.stringify(sudokuGame));
   ...
   for(var i in this.playersList){
     ...
     if(sudokuGame.sizeLimit() && this.playersList[i].avalible) {
     	...
     	this.playersList[i].setPlayerGameId(gameId);
     	this.playersList[i].setUnsolvedArray(sudokuArray);
     	console.log('this game id in create game ++ ' + this.playersList[i].gameId);
     	sudokuGame.addPlayer(this.playersList[i]);
     	console.log(this.playersList[i].getName() + " has joined the game");
     }
   }
...
Lobby.prototype.checkForWinner = function(){
    ...
    {  
    	...
    	console.log("in fun checkForWinner game list. lenght " + this.gameList.length);
    }
}
